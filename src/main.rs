...
use crate::{archive::upload, auth::{create_user, delete, login, logout}, routes::{add_playlist, del_playlist, index, loginp, playlistp, search, settings, signin, uploadp}};
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(index).service(create_user).service(login).service(logout).service(signin).service(loginp).service(uploadp).service(upload).service(search).service(add_playlist).service(del_playlist).service(playlistp).service(settings).service(delete)
    })
